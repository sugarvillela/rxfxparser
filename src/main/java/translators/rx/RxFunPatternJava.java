...
import translators.interfaces.RxFunPatternGen;
...
public class RxFunPatternJava implements RxFunPatternGen {
    ...
    @Override
    public RxFunPatternGen setNameGen(NameGenRx nameGen){
    ...
    private void genFunClassNames(PayNodes.RxPayNode[] funList){
        for(PayNodes.RxPayNode node : funList){
            String upperCaseName = nameGen.getRx(NameGenRx.RX_FUN);   // Screaming snake case
    ...
    private void genFunPatternClassBody(Keywords.OP op){
        String upperCaseName = nameGen.getRx(NameGenRx.RX_FUN_PATTERN);
