 ...
 * @author Dave Swanson
 */
public class ContextRxAction {
    private static ContextRxAction instance;
    
    public static ContextRxAction getInstance(){
        return (instance == null)? (instance = new ContextRxAction()) : instance;
    }
    
    public final RangeStripper RANGE;
    
    private final Pattern DUP_SYMBOLS = Pattern.compile(".*(&&|\\|\\||==|~~).*");
    private final Pattern BAD_BRACKET = Pattern.compile(".*(\\)\\(|\\}\\{).*");
    
    private ContextRxAction(){
        RANGE = new RangeStripper();
    }
    
    public boolean assertValRxWord(String text){
